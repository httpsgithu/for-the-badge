...
export interface DenyBadgeRequest {
    submissionId: string;
}

export interface DenyBadgeResponse {
    message: string;
}
...
export default defineEventHandler(async (event) =>
{
    ...
    const body = await readBody<DenyBadgeRequest>(event);
    ...
    try
    {
        ...
        // Update the submission status to denied
        await db
            .update(tables.submittedBadges)
            .set({
                status: "denied",
                reviewedAt: new Date(),
                reviewerQueryId: auth.queryId.cloneBytes(),
            })
            .where(eq(tables.submittedBadges.submissionId, submissionIdBytes))
            .run();

        return {
            message: "Badge denied successfully",
        } as DenyBadgeResponse;
    }
    catch (error)
    {
        ...
        console.error("Error denying badge:", error);
        throw createError({
            statusCode: 500,
